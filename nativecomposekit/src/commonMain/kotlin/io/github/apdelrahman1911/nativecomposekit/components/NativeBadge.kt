 ...
 * unread red (`error`); pass [containerColor]/[contentColor] (e.g. `NativeTheme.statusColors.success`) for a
 * semantic badge. A numbered badge announces its count to screen readers by default — pass a contextual
 * [contentDescription] ("5 unread") when the bare number is ambiguous. Overlay it with [NativeBadgedBox].
 */
@Composable
public fun NativeBadge(
...
/**
 * Whether [NativeBadge] renders anything for [count]: a null count is a dot (visible), a positive count is a
 ...
 *
 * `NativeBadgedBox(badge = { NativeBadge(count = 3) }) { Icon(...) }`
 */
@Composable
public fun NativeBadgedBox(
