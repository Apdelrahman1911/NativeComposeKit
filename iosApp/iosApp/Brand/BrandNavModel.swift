...
/// The SwiftUI-side projection of the Kotlin `BrandNavigator` (the source of truth), driven through
/// `BrandNavBridge`. It mirrors the SoT into `@Published` state (SoT → SwiftUI via `observe`) and reports user
...
/// `DispatchQueue.main`).
final class BrandNavModel: ObservableObject {
    ...
    let tabIds: [String]
    private let bridge: BrandNavBridge
    private var cancellable: BrandNavCancellable?
