 ...
 * AppTheme). Wrap the anchor as [content]; [text] is the tip. Distinct from [BrandPopover] (a richer,
 * caller-controlled surface): a tooltip is a short, self-dismissing system-style hint.
 *
 * `BrandTooltip("Add to library") { BrandIconButton(addIcon, ::add, contentDescription = "Add") }`
 *
 ...
 * value. Prefer inline helper text, an accessible `contentDescription`, or a [BrandPopover] for caller-controlled
...
@Deprecated(
    message = "Material-only wrapper with no native iOS idiom (long-press = context menu on iOS). Prefer inline " +
        "helper text / contentDescription, or BrandPopover for caller-controlled contextual content.",
    level = DeprecationLevel.WARNING,
)
@OptIn(ExperimentalMaterial3Api::class)
@Composable
public fun BrandTooltip(
    text: String,
