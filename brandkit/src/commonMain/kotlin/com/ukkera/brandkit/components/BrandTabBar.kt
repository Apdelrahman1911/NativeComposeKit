 ...
 * distinct from the app's bottom tab bar (that's the native `TabView`/`BrandNavHost` shell). **Compose-drawn
 ...
 *
 * `BrandTabBar(listOf("Overview","Chapters"), selectedIndex = tab, onSelectedIndexChange = { tab = it })`
 *
 ...
 * already covered natively by [BrandSegmentedControl] (a real `UISegmentedControl` on iOS), which is the
 * idiomatic iOS in-content switcher. Migrate to [BrandSegmentedControl]; this will be removed in a later release.
 */
@Deprecated(
    message = "Material-only wrapper with no native iOS renderer. Use BrandSegmentedControl (native " +
        "UISegmentedControl on iOS) for in-content selection.",
    replaceWith = ReplaceWith(
        "BrandSegmentedControl(options = tabs, selectedIndex = selectedIndex, " +
