 ...
 */
public enum class BrandSelectionStyle { Radio, Checkmark }
 ...
 * - [BrandSelectionStyle.Checkmark] renders the iOS grouped-table checkmark-list idiom (use this on iOS);
 * - for a small, fixed option set, prefer [BrandSegmentedControl] (a real `UISegmentedControl` on iOS).
 *
 * [BrandSelectionStyle.Radio] is the Material/Android dot idiom. Selection uses `==`, so [T] must have a stable
 * `equals` (a data class, enum, or primitive — not identity types like lambdas). [selected] = null is a valid
 * "nothing selected yet" state.
 *
 * `BrandRadioGroup(qualities, selected = quality, onSelectedChange = { quality = it }, label = { it.name })`
 */
@Composable
public fun <T> BrandRadioGroup(
    options: List<T>,
    selected: T?,
    ...
    style: BrandSelectionStyle = BrandSelectionStyle.Radio,
    testTag: String? = null,
) {
