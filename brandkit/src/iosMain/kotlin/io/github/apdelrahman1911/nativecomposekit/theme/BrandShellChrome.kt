 ...
 * background) for a native host that must paint a background SwiftUI-side. [applyBrandShellChrome] already
 ...
 * `.background(...)`. Build a dynamic `UIColor { brandBackgroundUIColor(dark: $0.userInterfaceStyle == .dark) }`
 * in Swift so it adapts to light/dark.
 */
public fun brandBackgroundUIColor(dark: Boolean): UIColor =
    (if (dark) brandDarkBackground else brandLightBackground).toUIColor()
 ...
 *
 * This is the ROOT fix and it lives in the kit: it's driven automatically by [BrandAppearanceScope] (a
 * `LaunchedEffect` on the resolved dark state), so it runs at launch AND on every light↔dark flip — system
 * or in-app ([BrandAppearance.setDark]) — because Compose recomposes either way. A host app gets seamless
 * chrome for free with **no Swift code** (except the floating-tab-bar surround; see [brandBackgroundUIColor]).
 */
internal actual fun applyBrandShellChrome(dark: Boolean) {
    val bg = brandBackgroundUIColor(dark)
